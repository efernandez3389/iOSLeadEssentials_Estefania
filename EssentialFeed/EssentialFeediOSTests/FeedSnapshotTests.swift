...
//
//  Created by Estefania Fernandez on 22/7/24.
//
...
class FeedSnapshotTests: XCTestCase {
    ...
    private func assert(snapshot: UIImage, named name: String, file: StaticString = #file, line: UInt = #line) {
        ...
        guard let storedSnapshotData = try? Data(contentsOf: snapshotURL) else {
            XCTFail("Failed to load stored snapshot ar URL: \(snapshotURL). Use the 'record' method to store a snapshot before asserting.", file: file, line: line)
            return
        }
    ...
    private func record(snapshot: UIImage, named name: String, file: StaticString = #file, line: UInt = #line) {
        ...
        do {
            ...
            XCTFail("Fail to record snapshot with error: \(error)", file: file, line: line)
        }
